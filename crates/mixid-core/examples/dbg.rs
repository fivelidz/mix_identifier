...
fn main() {
    let a = fingerprint_file(Path::new("test_data/songs/songA.wav"))
        .unwrap()
        .0;
    let c = fingerprint_file(Path::new("test_data/songs/songB.wav"))
        .unwrap()
        .0;
    let e = fingerprint_file(Path::new("test_data/songs/songE.wav"))
        .unwrap()
        .0;

    let mut ma: HashMap<u32, Vec<u32>> = HashMap::new();
    for &(h, t) in &a.hashes {
        ma.entry(h).or_default().push(t);
    }
    let mut mc: HashMap<u32, Vec<u32>> = HashMap::new();
    for &(h, t) in &c.hashes {
        mc.entry(h).or_default().push(t);
    }
    let mut me: HashMap<u32, Vec<u32>> = HashMap::new();
    for &(h, t) in &e.hashes {
        me.entry(h).or_default().push(t);
    }
    ...
    println!(
        "shared A∩C distinct hashes: {} ({:.1}% of A)",
        shared_ac,
        100.0 * shared_ac as f64 / ma.len() as f64
    );
    println!(
        "shared A∩E distinct hashes: {} ({:.1}% of A)",
        shared_ae,
        100.0 * shared_ae as f64 / ma.len() as f64
    );

    // decode top shared hashes
    let mut shared: Vec<(u32, usize)> = ma
        .iter()
    ...
    for (h, n) in shared.iter().take(15) {
        ...
        let dt = h & 0x3F;
        println!(
            "  f1={:3} ({:5.0} Hz)  f2={:3} ({:5.0} Hz)  dt={:2}  occ={}",
            f1,
            f1 as f64 * 10.766,
            f2,
            f2 as f64 * 10.766,
...
fn analyze_clusters(label: &str, mix: &mixid_core::Fingerprint, track: &mixid_core::Fingerprint) {
    ...
    let mut v: Vec<(i64, u32, usize)> = votes
        .into_iter()
        .map(|(o, (n, s))| (o, n, s.len()))
        .collect();
    ...
    println!(
        "{label}: track distinct hashes (capped): {}",
        track_distinct
    );
}
